...
void Block::setTextBlock() 
{
  ...
  if (_text.length() == 1) 
    ...
    //Locator::getDisplay()->setCursor(_x + 6, 20);
    }  
  else 
    {
    Locator::getDisplay()->setCursor(_x + 2, _y + 12);
    //Locator::getDisplay()->setCursor(_x + 2, 20);
    }
...
void Block::update() 
{
  ...
  if (_state == IDLE && _lastState != _state) 
    ...
    if (millis() - lastMillis >= 60) 
      {     
      Locator::getDisplay()->fillRect(_x, _y, _width, _height, SKY_COLOR);     // Comment these if you want no Clock Jump
      _y = _y + (MOVE_PACE * (direction == UP ? -1 : 1));
      Locator::getDisplay()->drawRGBBitmap(_x, _y, BLOCK, _width, _height);    // Comment these if you want no Clock Jump
      setTextBlock();
